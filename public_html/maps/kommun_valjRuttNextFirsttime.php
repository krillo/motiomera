	...
	<data>
		...
		while(list($key,$stracka) = each($rutten)) {
			if($key > $sista) {
				...
				if(array_search($stracka["Kommun"],$grannkommuner) !== false)
					{?>link="javascript:motiomera_ruttLaggTill(<?=$stracka["Kommun"]->getId()?>)" tooltext="Klicka för att gå till <?=utf8_decode($stracka["Kommun"]->getNamn())?>!"<?}
				?> />
					<?php
				}
		...
		while(list($key,$kommun) = each($grannkommuner)) {
			if(array_search($kommun->getKod(),$exclude) === false)
				{?><entity id= '<?=$kommun->getKod()?>' <?//color='#FFEE00'?> link="javascript:motiomera_ruttLaggTill(<?=$kommun->getId()?>)" tooltext="Klicka för att gå till <?=utf8_decode($kommun->getNamn())?>!"/> <?php }	   		
