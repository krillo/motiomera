...
class MotiomeraMail extends Mobject {
   ...
   * @return type 
   */
  public static function getUnreadMail($id) {
    global $db;
    if (!empty($id)) {
      $sql = "SELECT count(id) FROM mm_motiomeramail WHERE is_read = 0 and to_id = $id";
      return $db->value($sql);
    } else {
      return '';
    }
  ...
  public function setAllowLinks($arg) {
    ...
    if ($arg != 0)
      $this->allow_links = 'true';
    else
      $this->allow_links = 'false';
  }
