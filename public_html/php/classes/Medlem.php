...
#			}
  ...
  public function setLosenord($losenord) {
    ...
    if (strlen($losenord) < self::MIN_LENGTH_LOSEN)
      throw new MedlemException("Lösenordet är för kort", -4);
  ...
  public function setVisningsbild($visningsbild) {
    ...
    if (get_class($this->getVisningsbild()) == "CustomVisningsbild")
      $this->getVisningsbild()->delete();
  ...
  public function setAtkomst($atkomst) {
    ...
    if ($this->getId()) {
      ...
      if (!in_array($atkomst, array(
                  ...
                  "foretag",
                  "ingen"
              )) || ($this->getForetag() && $atkomst == "ingen") || (!$this->getForetag() && $atkomst == "foretag"))
        throw new MedlemException($atkomst . ' har ett felaktigt värde', -10);
    }
  ...
  public function setPaidUntil($datum) {
    ...
    if (!Misc::isDate($datum, "Y-m-d"))
      throw new MedlemException('$datum har ett felaktigt format', -18);
  ...
  public function setMotiomeraMailBlock($var) {
    /** true or false */
    if ($var == 'true')
      $this->block_mail = 'true';
    elseif ($var == 'false')
      $this->block_mail = 'false';
  }
  ...
  public function removeAllSteg() { //** Security check before calling this function in actions/delete
    global $db;
    $sql = 'DELETE FROM ' . Steg::TABLE . ' WHERE medlem_id = ' . $this->id;
    $db->query($sql);
  }

  public function removeAllStrackor() { //** Security check before calling this function in actions/delete
    global $db;
  ...
  public static function loadByUsername($username) {
    ...
    try {
      ...
      if ($e->getCode() == - 2)
        throw new MedlemException("Användarnamnet kunde inte hittas", -21);
    }
    return $medlem;
  }
