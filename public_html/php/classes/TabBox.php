...
class TabBox {
  ...
  public function printTabBox() {
    ...
    <?php foreach ($this->getTabs() as $i => $name) { ?>
          <div class="mmTabBoxTab<?php if ($this->getSelected() == $i) echo "Selected"; ?>" id="<?php echo $this->getId() ?>_tab_<?php echo $i ?>">
            <table border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td class="mmTabBoxLeft"></td>
                <td class="mmTabBoxMid"><a href="#" onclick="mmTabBoxChangeTab('<?php echo $this->getId() ?>', <?php echo $i ?>, <?php echo $this->getAjaxTab($i) ?>);
                    return false;"><?php echo $name ?></a></td>
      ...
      <div class="mmTabBoxContent" style="height: <?php echo $this->getHeight() ?>px;width: <?php echo $this->getWidth() + 5; ?>px;overflow:hidden;">
          ...
          <div id="<?php echo $this->getId() ?>_content_<?php echo $i ?>"<?php echo ($i != $this->getSelected()) ? ' class="hide"' : ""; ?>><?php
            if (!file_exists($this->getUrl($i)))
              echo "File not found";
            else
            if ($this->ajaxTabs[$i] == 0 || $this->getSelected() == $i)
              include ($this->getUrl($i));
            ?></div>
        <?php }
        ?>
      </div>
