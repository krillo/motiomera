...
{	
   ...
   */
	...
	public static function giltigForetagsnyckel($nyckel)
	{
		...
		if (!$row) return self::FN_OGILTIG;
		else {
			
			if ($row["medlem_id"] != "") return self::FN_UPPTAGEN;
			else return true;
		}
	}
	...
	public static function loadByForetagsnyckel($nyckel)
	{
		...
		if (!self::giltigForetagsnyckel($nyckel)) throw new ForetagException("Ogiltig f??agsnyckel", -7);
 ...
 */	
	...
	public function isGiltig()
	{
		...
		if (strtotime($this->getGiltig()) < strtotime(date("Y-m-d"))) return false;
		else return true;
	}
	...
	public function gaMedI($nyckel, $medlem = null)
	{
		...
		if ($value == 0) throw new ForetagException("Ogiltig nyckel", -6);
  ...
  public function getSumReclamations()
  {
	...
	public static function getTopplistaForetag($limit = null)
	{
		...
		foreach($foretagList as $row) {
			...
			if ($row['id'] != 0) $topplista[] = Foretag::loadById($row['id']);
		}
  ...
  public function getUpdatedDate()
  {
	...
	public function listNycklar($free = false){		
		if (!$this->nycklar) {
			...
			if ($free) $sql.= " AND medlem_id is null";
			$res = $db->query($sql);
			$result = array();
	...
	public function listLag(){		
		if (!$this->lag) $this->lag = Lag::listByForetag($this);
		return $this->lag;
	}
	...
	{		
		if ($this->id) Security::demand(FORETAG, $this);
	...
	{;
		...
		if ($medlem->getId() != $USER->getId()) Security::demand(ADMIN);
		
		if (!Misc::isint($medlem_id)) throw new ForetagException('$medlem_id m䲴e vara ett heltal', -2);
	...
	public function setANamn($aNamn)
	{
		...
		if ($this->getId()) Security::demand(ADMIN);
		global $db;
		$sql = "SELECT count(*) from " . self::classToTable(get_class()) . " WHERE aNamn = '" . Security::secure_data($aNamn) . "'";
		
		if ($this->getId()) $sql.= " AND id <> " . $this->getId();
		
		if ($db->value($sql) > 0) throw new ForetagException("Anv㭤arnamnet 㰠upptaget", -4);
		$this->aNamn = $aNamn;
	}
	...
	public function setGiltig($datum)
	{
		...
		if (!Misc::isDate($datum, "Y-m-d")) throw new ForetagException('$datum har ett felaktigt format', -8);
		$this->giltig = $datum;
	}
