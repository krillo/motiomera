...
class UploadProgressMeter {
	...
	var $formExtra = 'onsubmit="fotoalbumRensaUpp(); UploadProgressMeter_Start(this); return true;" target="[frame_name]"';

	var $formIframe = "<iframe id='[frame_name]' name='[frame_name]' src='' style='width:1px;height:1px;border:0'></iframe>";

	var $progressBarDiv = '<div id="[id]" class="progressBar hide"><div class="background"><div class="bar">&nbsp;</div></div><div class="message"></div></div><script type="text/javascript">UploadProgressMeter_Register(\'[id]\',\'[upload_id]\')</script>';
	...
	function enableDebug() {
		$this->formIframe = "Form Submision Iframe Target<br><iframe id='debug' name='[frame_name]' src='' style='position: absolute; top: 270px; width: 400px; height: 150px; border: 1'></iframe><br>";
	}
	...
	function finalStatus() {
		$error = false;
		if (count($_FILES) == 0) {
			$error = "Uppladdning misslyckades, okänt fel.<br />";
		}
		foreach($_FILES as $file) {
			switch($file['error']) {
				...
				case 2: // exceded max file size
					$error = "Uppladdning misslyckades, filen är för stor";
					break;
				case 3: // partial file
					$error = "Uppladdninge misslyckades, var god försök igen";
					break;
				case 4: // no file
					$error = "Uppladdning misslyckades, var god försök igen";
					break;
				case 6: // no tmp dir
				case 7: // can't write to tmp
					$error = "Uppladdning misslyckades, internt fel";
					break;
			}
		}
