...
<div class="mmFloatRight">
	...
	<br />
		...
		{*}<table width="155" cellpadding="0" cellspacing="0" border="0">
			...
			{foreach name=steglista from=$topplistaForetag item=foretag}
			
			<tr>
				<td>{$smarty.foreach.steglista.iteration}.</td>
				<td><a href="{$urlHandler->getUrl(Foretag, URL_VIEW, $foretag->getId())}">{$foretag->getNamn()}</a></td>
				<td>{$foretag->getStegTotal()|nice_tal}</td>
			</tr>
