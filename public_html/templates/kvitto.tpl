...
{elseif $orderList.typ == "foretag"}
  ...
  {if $orderList.typ == "foretag"}
    ...
    Som tävlingsansvarig har du tillgång till din egen administrationssida,<span class="mm-alert-text">http://motiomera.se/foretaglogin.</span> På denna sida kan du göra din egen lagindelning samt göra tilläggsbeställningar. Se dina inloggningsuppgifter nedan.
  ...
  {if $orderList.typ == "foretag"} 
    ...
    <div>
      <div class="mm-alert-box" id="mm-kvitto-cred" style="float:left;width:300px;">
        <h2>Inloggningsuppgifter</h2>
        <b>Loginsida:</b> <a href="/pages/foretaglogin.php?u={$orderList.foretagANamn}&p={$orderList.foretagLosen}">http://motiomera.se/foretaglogin</a><br />
  ...
  <table border="0" cellpadding="0" cellspacing="0" id="mmKvittoTable" class="mm-alert-box">
    {if $orderList.typ == "foretag" || $orderList.typ == "foretag_again" || $orderList.typ == "foretag_tillagg"}
      ...
      <tr>
        <td class="mmWidthTvaHundraPixlar"><b>Köpet genomfört:</b><br/> {$orderList.date|date_format:"%Y-%m-%d %H:%m"}</td>
